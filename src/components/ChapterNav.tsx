...
export default function ChapterNav({ currentChapter, onChapterClick }: ChapterNavProps) {
  ...
  return (
    ...
    >
        ...
        {chapters.map((chapter, index) => (
          <div key={chapter.id} className="relative">
            ...
            <button
              ...
              className={`chapter-nav-dot relative w-3 h-3 rounded-full transition-all duration-500 ${
            ...
            >
              ...
              {currentChapter === index && (
                <>
                  <span className="absolute inset-0 rounded-full bg-champagne-gold animate-ping opacity-20" />
                  <span className="absolute inset-[-4px] rounded-full border border-champagne-gold/30 animate-pulse" />
                </>
              )}
            </button>
      ...
      >
        <span className="font-mono text-xs text-soft-white/40">
